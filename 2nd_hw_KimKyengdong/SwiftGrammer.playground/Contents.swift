...
//
func printArea(_ garo:Int, _ sero:Int)->Int{
    return garo*sero
}
func printArea(_ half:Float)->Float{
...
func checkLoginStatus_if(_ isLogin:Bool, _ userName:String?)->String{
    if isLogin{
...
func checkLoginStatus_guard(_ isLogin:Bool, _ userName:String?)->String{
    guard isLogin else {
        return "로그인이 필요합니다."
    }
...
let temp=names.first ?? "No name found"
print("\(temp)")
...
func optionalCheck(_ optionalString:String?){
    guard let optionalString=optionalString else {
        print("The string is nil.")
        return
    }
    print("The string is: \(optionalString)")
