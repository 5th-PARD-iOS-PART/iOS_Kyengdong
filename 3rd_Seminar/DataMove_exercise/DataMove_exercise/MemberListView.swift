    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Button("저장") {
                ...
                guard let ageInt=Int(age), name.isEmpty==false, part.isEmpty==false else {
                    return
                }
           
                // 2. 수정 모드일 경우 기존 멤버 수정
                if let index = editIndex{
                    store.members[index] = .init(name: name, age: ageInt, part: part)
                }
                // 3. 추가 모드일 경우 새 멤버 추가
                else {
                    store.members.append(.init(name: name, age: ageInt, part: part))
                }
